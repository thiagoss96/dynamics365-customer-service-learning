...
Embora a venda seja fechada em um único contrato (Cotação), a operação logística é o grande desafio. Se a empresa enviar todas as unidades de uma vez:
...
## 🗄️ 1. Arquitetura de Dados (Dataverse Schema)
...
### A. Entidade Principal: Ordem de Remessa (`dat_ordemderemessa`)

| Nome Lógico | Nome de Exibição | Tipo de Dado | Descrição Técnica |
| :--- | :--- | :--- | :--- |
| `dat_ordemremessaid` | ID Remessa | Unique Identifier | **Primary Key (GUID)** do registro. |
| `dat_name` | ID da Remessa | String | Identificador legível (ex: REM-001). |
| `dat_statusdalinha` | Status da Linha | Choice (OptionSet) | `1`: Pendente, `2`: Em Processo, `3`: Concluído. |
| `dat_prazolimite` | Prazo Limite | Date and Time | Parâmetro de validação para o motor de SLA. |
| `_dat_quote_value` | Cotação | Lookup (N:1) | Foreign Key (FK) para a tabela nativa `quote`. |
| `_dat_plano_value` | Plano | Lookup (N:1) | Relacionamento (FK) com a tabela nativa de Produtos (`product`). |
| `dat_quantidade` | Quantidade | Whole Number | Campo numérico inteiro para controle de volume da remessa. |
...
* **Campo Lookup Customizado:** `dat_ordemderemessa` (Entity Reference).
...
## 🖼️ 2. Interface e Lógica Formulário de Cotação (Dynamics 365)
...
* **Trigger Field:** `dat_tipodotermo` (Tipo de Termo).

#### Lógica de Execução (State Control):


* **Cenário: Seleção "Termo de Cronograma"**
    * **IF:** `dat_tipodotermo` Equals `Termo de Cronograma`.
    * **THEN (Actions):**
...
* **Entidade Relacionada:** `dat_ordemderemessa`.
...
* **Lógica do Filtro:** Busca apenas registros ativos (`statuscode eq 1`), que ainda estão pendentes (`dat_statusdalinha eq 1`) e cujo prazo limite está entre hoje e os próximos 5 dias.
* **Expressão utilizada:** `dat_prazolimite le '@{addDays(utcNow(), 5, 'yyyy-MM-dd')}' and dat_prazolimite ge '@{utcNow('yyyy-MM-dd')}'`
...
* **Obter Cotação (Lookup):** Utiliza o ID `_dat_cotacao_value` vindo da remessa para buscar detalhes da cotação pai (como o Nome do Cliente e a Lista de Preços).
* **Obter Produto (Lookup):** Utiliza o ID `_dat_plano_value` para buscar metadados do produto, especificamente o **Código Fiscal / Código SAP**, que é essencial para o operacional.
...
Job automático que após a resolução de um chamado, verifica se a ocorrência criada pelo gatilho possui o guid da remessa salvo no campo: dat_ordemderemessa da tabela OcorrÊncia, se sim, ela irá atualizar o status da linha da Remessa que teve seu chamado encerrado com status: Concluída.
Verifica a resolução de pendências externas para encerrar o ciclo de vida do registro pai.
* **OData Query:** `statecode eq 1 and _dat_ordemderemessa_value ne null`
...
1.  **Contextualização (Data Fetching):**
    * **Obter Cotação/Termo:** O fluxo recupera o registro pai (`quote`) e o modelo de termo vinculado (`_dat_tipo_termo_value`).
...
2.  **Processamento de Itens (Looping & Logic):**
    ...
    * **Expressão de Tratamento de Data (SLA):**
        ```text
        if(empty(item()?['dat_prazolimite']), '---', formatDateTime(convertTimeZone(item()?['dat_prazolimite'], 'UTC', 'E. South America Standard Time'), 'dd/MM/yyyy'))
        ```
    * **Append Dinâmico:** Construção da estrutura `<tr>` injetando o `FormattedValue` (Display Name) dos campos de Lookup (`_dat_plano_value` e `_dat_cotacao_value`).
...
* **Coalesce:** Utilizado para garantir que o fluxo não quebre caso as observações estejam em branco: `coalesce(outputs('Obter_Cotação')?['body/dat_observacaodopedido'], '')`.
